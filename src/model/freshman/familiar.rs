...
use crate::model::freshman::{FreshmanAnalysis, MapDefaultAvatar};
...
impl FreshmanBasic {
    ...
    pub async fn post_analysis_log_model(&self, pool: &PgPool) -> Result<()> {
    ...
    async fn count_by_name(&self, client: &PgPool) -> Result<i64> {
        let same_name_count: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM freshman.students WHERE name = $1")
            .bind(&self.name)
            .fetch_one(client)
            .await?;
    ...
    async fn count_by_major(&self, client: &PgPool) -> Result<i64> {
        let major_count: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM freshman.students WHERE major = $1")
            .bind(&self.major)
            .fetch_one(client)
            .await?;
        Ok(major_count.0)
    ...
    async fn count_by_college(&self, client: &PgPool) -> Result<i64> {
        let college_count: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM freshman.students WHERE college = $1")
            .bind(&self.college)
            .fetch_one(client)
            .await?;
        Ok(college_count.0)
